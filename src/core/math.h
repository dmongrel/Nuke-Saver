...
// General 4x4 inverse by cofactor expansion. Needed to turn clip space back into world space
// for the sky pass, which reconstructs a view ray per pixel. A specialised affine inverse would
// not do: the matrix being inverted is view times projection, which is not affine.
//
// Returns the identity for a singular matrix rather than filling the scene with NaN.
inline Mat4 Inverse(const Mat4& src) {
    ...
    Mat4        out;
    float*      o = &out.m[0][0];
    for (int i = 0; i < 16; ++i) o[i] = inv[i] * rcp;
    return out;
}
