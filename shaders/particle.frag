...
void main() {
    ...
    if (vTint.w > 0.5) {
        ...
        vec3  normal   = viewDir;
        float keyAbove = smoothstep(-0.08, 0.06, scene.keyDirection.y);
        ...
        vec3 lit = vTint.rgb * scene.keyColor.rgb * wrap * wrap * keyAbove;
        ...
        lit += vTint.rgb * scene.keyColor.rgb * (forward * forward * forward * 1.4 * keyAbove);
